...
class LightGCN(nn.Module):
    ...
    def __init__(self, n_user, n_item, norm_adj):
        ...
        *********************************************************
        Get sparse adj.
        """
        self.sparse_norm_adj = self._convert_sp_mat_to_sp_tensor(#矩阵转张量
    ...
    def computer(self, drop_flag=False):
        ...
        for layer in range(len(self.layers)):
            all_emb = torch.sparse.mm(g_droped, all_emb)#稀疏矩阵与权重矩阵相乘
            #             embs.append(all_emb * self.alpha_K[layer])# ʵ��1/(k+1)
            embs.append(all_emb)
        embs = torch.stack(embs, dim=1)
        #         print(all_emb.size())
        #         print(embs.size())
        light_out = torch.mean(embs, dim=1)  #����������1/(k+1)��
